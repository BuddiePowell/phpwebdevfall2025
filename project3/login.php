...
if (empty($_SESSION['user_id']) && isset($_POST['login']))
{
    ...
    if (!empty($user_name) && !empty($password))
    {
        ...
        // Check if user exists in exercise_user table  // Fixed typo in comment
        $query = "SELECT id, user_name, password_hash FROM exercise_user WHERE user_name = ?";
        ...
        if (mysqli_num_rows($results) == 1)
        {
            ...
            if (password_verify($password, $row['password_hash']))
            {
                ...
                // Redirect to the home page (update to a specific page if needed)
                header('Location: index.php');  // Example: redirect to index.php
...
?>
        ...
        <?php
         elseif (isset($_SESSION['user_name'])): ?>
               <h4><p class='text-success'>You are logged in as: <strong><?php echo htmlspecialchars($_SESSION['user_name']); ?></strong>.</p></h4>
           <?php endif; ?> 
      </div>
    </div>
  <script>
